...
import { getWallet, linkBankAccount, unlinkBankAccount, topUpWallet, getTransactions } from '../../services/api';
...
export default function MyWallet() {
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      setShowTopUp(false);
      setTopUpAmount('');
      setSuccess(`Nạp ${amount.toLocaleString()}đ thành công!`);
      setTimeout(() => setSuccess(''), 3000);
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-gradient-to-br from-slate-50 to-indigo-50 p-4 md:p-6">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Balance Card */}
        <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-6 mb-6 text-white shadow-xl animate-fade-in">
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowTopUp(true)} className="flex-1 py-3 bg-white/20 hover:bg-white/30 rounded-xl font-medium transition-colors flex items-center justify-center gap-2">
              <span>💳</span> Nạp tiền
            </button>
            {wallet?.isLinked ? (
                  ...
                  <div key={i} className={`flex items-center justify-between p-4 ${config.bg} rounded-xl`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{t.description || 'Không có mô tả'}</p>
            ...
            <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select value={bankForm.bankName} onChange={(e) => setBankForm({...bankForm, bankName: e.target.value})} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                  ...
                  <input type="text" placeholder="Nhập số tài khoản" value={bankForm.accountNumber} onChange={(e) => setBankForm({...bankForm, accountNumber: e.target.value})} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tên chủ tài khoản</label>
                  <input type="text" placeholder="Nhập tên chủ tài khoản" value={bankForm.accountHolder} onChange={(e) => setBankForm({...bankForm, accountHolder: e.target.value})} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
                <div className="flex gap-3 pt-2">
                  <button onClick={() => { setShowLinkBank(false); setError(''); }} className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition-colors">Hủy</button>
                  <button onClick={handleLinkBank} disabled={actionLoading} className="flex-1 py-3 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Số tiền nạp (VNĐ)</label>
                  <input type="number" placeholder="Nhập số tiền" value={topUpAmount} onChange={(e) => setTopUpAmount(e.target.value)} className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 text-lg" />
                </div>
                <div className="flex gap-2 flex-wrap">
                  {[50000, 100000, 200000, 500000, 1000000].map(amt => (
                    <button key={amt} onClick={() => setTopUpAmount(amt.toString())} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${topUpAmount === amt.toString() ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-green-100'}`}>
                ...
                {topUpAmount && (
                  <div className="p-4 bg-green-50 rounded-xl text-center">
                    <p className="text-sm text-gray-600">Số tiền nạp</p>
                    <p className="text-3xl font-bold text-green-600">{parseInt(topUpAmount || 0).toLocaleString()}đ</p>
                  </div>
                )}
                <div className="flex gap-3 pt-2">
                  <button onClick={() => { setShowTopUp(false); setError(''); setTopUpAmount(''); }} className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition-colors">Hủy</button>
                  <button onClick={handleTopUp} disabled={actionLoading || !topUpAmount} className="flex-1 py-3 bg-green-500 text-white rounded-xl font-medium hover:bg-green-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                    {actionLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Nạp tiền'}
                  </button>
                </div>
              </div>
            </div>
